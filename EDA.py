...
DATASET_DIR = '/Users/vini/Desktop/Autonomous_Driving'
...
def plot_figures(
    ...
    pielist = ax.pie(sizes, explode=explode, labels=labels, autopct='%1.1f%%', shadow=False, startangle=start_angle,
                     counterclock=False)
    ax.axis('equal')
...
def show_samples(samples):
    for sample in samples:
        ...
        for contour in contours:
            poly_patch = Polygon(contour.reshape(-1, 2), closed=True, linewidth=2, edgecolor='r', facecolor='r',
                                 fill=True)
